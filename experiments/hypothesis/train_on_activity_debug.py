...
if __name__ == "__main__":
    ...
    activity_data = ActivityDataModule(
        data_path=Path("../../data"),
        save_dir=Path("act_data/ups"),
        **dict(data_config.data)
    )
    activity_data.prepare_data()
    activity_data.setup("fit")
    for i in range(len(activity_data.train_dataset)):
        batch = activity_data.train_dataset[i]
        traj = batch[0]
        if torch.allclose(traj[0], torch.zeros_like(traj)):
            print(i, torch.allclose(traj, torch.zeros_like(traj)))

    print(len(activity_data.train_dataset))
    pass
    # logger = WandbLogger(
