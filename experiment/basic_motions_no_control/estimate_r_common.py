...
if __name__ == "__main__":
    ...
    mlflow.start_run(run_name="estimate_r")

    # R is measurement-noise covariance; with H = I and Q ~= 0 it equals the
    # covariance of the field's one-step prediction error. It is a property of
    # the sensor, not of the class, so estimate a SINGLE global R by pooling the
    # one-step innovations of every class' own field on its own TRAIN split
    # (where each field is well fit).
    innovations = []
    d = None
    for label in config.classes:
        train_dataset = TrajectoryDataset(
            config.train_path, label,
        ...
        # accumulate forward-filter innovations e_i = z_i - x_{i|i-1} over every
        # training trajectory; the innovation is exactly what R models (H = I)
        for inst_idx in track(range(len(train_dataset.trajs)), f"[{label}] train trajectory"):
    ...
    # single diagonal R (per-channel), pooled over all classes; ddof=1 for an
