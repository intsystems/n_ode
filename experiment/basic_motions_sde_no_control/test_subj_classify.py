...
if __name__ == "__main__":
    ...
    mlflow.set_experiment("basic_motions_sde_no_control")
    mlflow.start_run(
    ...
    R_est = (torch.eye(d) * 1e-3).numpy().astype(np.float32)
    ...
    for inst_idx in track(range(len(test_dataset.trajs)), "Test trajectory"):
        ...
        for label, field_module in list(label_fields.items()):
            ...
            # diffusion is a constant learnable per-channel sigma, so the
            # process noise Q = diag(sigma^2) * dt is stationary here
            sigma = field.brownian_sigma.detach().numpy()
            ukf.Q = np.diag(sigma ** 2) * config.dt
            ukf.R = R_est
            ...
            def fx_at(x, dt_, t):
                ...
                pred = odeint(
                    field.f, xt, t_eval, rtol=1e-6, atol=1e-6,
                    method="euler"
                )
                return pred[-1].numpy()
            ...
            # traj_covs_unnormed = [(field_module.traj_std.numpy() ** 2) * x_cov for x_cov in traj_covs]
            # loss = [
            #     (x - x_true) @ np.linalg.solve(x_cov, x - x_true)
            #     for x, x_true, x_cov in zip(traj_smooth_unnorm, traj.numpy(), traj_covs_unnormed)
            # ]
            # loss = sum(loss) / cur_traj.shape[0]
