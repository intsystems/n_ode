...
from hypothesis.len_fixed.components.field_model import MyVectorField
from hypothesis.len_fixed.components.field_module import LitNodeHype
...
if __name__ == "__main__":
    ...
    logger = WandbLogger(
        name=f"{args.act}-{args.subj_id}-" + generate_id(4),
        tags=["mlp_tanh", "mse", f"subj{args.subj_id}", args.act],
        config=dict(train_config),
        log_model="all",
        # it only attributes to wandb cloud
        checkpoint_name=f"subj_{args.subj_id}_{args.act}_checkpoint",
        **dict(wandb_config)
    )
    ...
    logger.use_artifact(f"subj_{args.subj_id}_{args.act}_train:latest")
    logger.use_artifact(f"subj_{args.subj_id}_{args.act}_test:latest")

    earlystopping_callback = EarlyStopping(**dict(train_config.early_stop))
    checkpoint_callback = ModelCheckpoint(
        dirpath=args.output_dir,
        filename=f"{args.act}_checkpoint",
