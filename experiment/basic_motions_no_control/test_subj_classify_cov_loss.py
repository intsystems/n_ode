...
from experiment.basic_motions_no_control.utils.dataset import TrajectoryDataset
from experiment.basic_motions_no_control.utils.field import FieldLitModule
...
if __name__ == "__main__":
    ...
    config = OmegaConf.load("experiment/basic_motions_no_control/config.yaml")

    mlflow.set_tracking_uri(config.tracking_uri)
    mlflow.set_experiment("basic_motions_no_control")
    mlflow.start_run(
    ...
    # single global measurement-noise covariance R, pooled across all classes
    # (see estimate_r_common.py); R is a sensor property, shared by every field
    # R_diag = pd.read_csv(
    #     os.path.join(config.results_dir, "R_diag.csv"), index_col=0
    # )["R_diag"].to_numpy()
    # R_est = np.diag(R_diag).astype(np.float32)
    ...
    for inst_idx in track(range(len(test_dataset.trajs)), "Test trajectory"):
        ...
        for label, field_module in list(label_fields.items()):
            ...
            def fx_at(x, dt_, t):
                ...
                pred = odeint(
                    field, xt, t_eval, method="rk4"
                )
                # odeint returns the state at every point in t_eval; take t + dt_
                return pred[-1].numpy()
            ...
            traj_smooth_norm, traj_covs, _ = rts_smoother_nonstationary(
                ukf, mu, cov, fx_at, config.dt
            )

            # covariance-weighted (Mahalanobis) reconstruction loss: each step's
            # squared error is scaled by the inverse smoothed covariance, so more
            # uncertain steps contribute less
            loss = [
                (x - x_true) @ np.linalg.solve(x_cov, x - x_true)
                for x, x_true, x_cov in zip(traj_smooth_norm, cur_traj, traj_covs)
            ]
            loss = float(sum(loss) / cur_traj.shape[0])
            ...
            for i, fig in enumerate(components_fig):
                # smoothed per-component std, mapped back to un-normalized units
                comp_sigma = np.sqrt(np.array([cov[i, i] for cov in traj_covs])) \
                    * field_module.traj_std.numpy()[i]
                fig.add_trace(go.Scatter(
                    x=t_mesh,
