...
if __name__ == "__main__":
    ...
    # (see estimate_r.py); R is a sensor property, shared by every candidate field
    # R_diag = pd.read_csv(
    ...
    for label in config.classes:
        ...
        # 
        # R_diag = pd.read_csv(
        #     os.path.join(config.results_dir, label, "R_diag.csv"), index_col=0
        # )["R_diag"].to_numpy()
        # label_Rs[label] = np.diag(R_diag).astype(np.float32)
