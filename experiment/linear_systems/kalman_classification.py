...
def compute_distance(pred_field, traj, dt, noise_sigma, x0_sigma):
...
if __name__ == "__main__":
    ...
    for target_field_indx, target_field in enumerate(candidate_fields):
        ...
        target_trajs += torch.randn_like(target_trajs) * config.noise_sigma
        ...
        for pred_field_indx, pred_field in enumerate(pred_fields):
            distance_tasks = [
                pool.submit(
                    ...
                    config.noise_sigma, config.x0_sigma
                )
